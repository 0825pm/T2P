...
def load_data(cfg: dict) -> (Dataset, Dataset, Optional[Dataset], Vocabulary, Vocabulary):
    ...
    cond_size = cfg["model"]["text_encoder"]["hidden_size"]
    ...
    reg_code_field = data.Field(sequential=True,
                               use_vocab=False,
                               dtype=torch.float32,
                               batch_first=True,
                               include_lengths=False,
                               pad_token=torch.ones((300,))*-1,
                               preprocessing=tokenize_features,
                               postprocessing=stack_features,)

    # Create the Training Data, using the SignProdDataset
    train_data = SignProdDataset(path=train_path,
                                 exts=("." + src_lang, "." + trg_lang, "." + files_lang, "." + cond_lang, "." + code_lang),
                                 fields=(src_field, reg_trg_field, files_field, text_field, reg_cond_field, reg_code_field),
                                 trg_size=trg_size,
                                 cond_size=cond_size,
                                 code_size=300,
...
class SignProdDataset(data.Dataset):
    ...
    def __init__(self, path, exts, fields, trg_size, cond_size, code_size, skip_frames=1, **kwargs):
        ...
        if not isinstance(fields[0], (tuple, list)):
            fields = [('src', fields[0]), ('trg', fields[1]), ('file_paths', fields[2]), ('text', fields[3]), ('cond', fields[4]), ('code', fields[5])]

        src_path, trg_path, file_path, cond_path, code_path = tuple(os.path.expanduser(path + x) for x in exts)
                ...
                io.open(trg_path, mode='r', encoding='utf-8') as trg_file, \
                    io.open(file_path, mode='r', encoding='utf-8') as files_file, \
                        io.open(cond_path, mode='r', encoding='utf-8') as cond_file, \
                            io.open(code_path, mode='r', encoding='utf-8') as code_file:

            i = 0
            # For Source, Target and FilePath
            for src_line, trg_line, files_line, cond_line, code_line in zip(src_file, trg_file, files_file, cond_file, code_file):
                i+= 1

                # Strip away the "\n" at the end of the line
                src_line, trg_line, files_line, cond_line, code_line = src_line.strip(), trg_line.strip(), files_line.strip(), cond_line.strip(), code_line.strip()
                ...
                cond_line = [(float(cond) + 1e-8) for cond in cond_line]
                code_line = [(int(code) + 1e-8) for code in code_line]
                ...
                cond_frames = [cond_line[i:i + cond_size] for i in range(0, len(cond_line), cond_size*skip_frames)]
                code_frames = [code_line[i:i + code_size] for i in range(0, len(code_line), code_size*skip_frames)]
                
                # Create a dataset examples out of the Source, Target Frames and FilesPath
                if src_line != '' and trg_line != '':
                    examples.append(data.Example.fromlist(
                        [src_line, trg_frames, files_line, src_line, cond_frames, code_frames], fields))
