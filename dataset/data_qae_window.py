...
def load_data(cfg: dict) -> (Dataset, Dataset, Optional[Dataset], Vocabulary, Vocabulary):
    ...
    min_trg_length = data_cfg.get("min_trg_length", 64)
...
class SignProdDataset(data.Dataset):
    ...
    def __init__(self, path, exts, fields, trg_size, skip_frames=1, window_size=64, phase='test', **kwargs):
        ...
        with io.open(src_path, mode='r', encoding='utf-8') as src_file, \
            ...
            for src_line, trg_line, files_line in zip(src_file, trg_file, files_file):
                ...
                if phase == 'train':
                    ...
                    if len(trg_frames) < window_size:
                        continue
                    else:
                        clip_poses = np.array(trg_frames)
                        idx = random.randint(0, clip_poses.shape[0] - window_size)
                        trg_frames = clip_poses[idx:idx + window_size].tolist()
