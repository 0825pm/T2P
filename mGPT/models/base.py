...
class BaseModel(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # 수정: get_rank_safe() 사용
        save_dir = os.path.join(self.output_dir, f'{self.hparams.cfg.TEST.SPLIT}_rank_'+str(get_rank_safe()))
    ...
    def save_npy(self, outputs):
        ...
        for output in outputs:
            name = output['name']
            feats_rst = output['feats_rst'].detach().cpu().numpy()
            feats_ref = output['feats_ref'].detach().cpu().numpy()
            lengths = output['lengths']
            lengths_rst = output['lengths_rst']
            
            for i in range(len(name)):
                cur_n = name[i].split('/')[-1]
                f_rst = feats_rst[i, :lengths_rst[i]]
                f_ref = feats_ref[i, :lengths[i]]
                np.save(os.path.join(output_dir, f'{cur_n}_rst.npy'), f_rst)
                np.save(os.path.join(output_dir, f'{cur_n}_ref.npy'), f_ref)