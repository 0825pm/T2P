...
def train(config, dataloader, model, src_vocab, optimizer, clip_grad_fun):
    ...
    for i, batch in enumerate(tqdm(dataloader)):
        ...
        pose_output, recon_loss, distill_loss = model(pose_input, text_input, pose_length)
        ...
        loss_all["distill_loss"].update(distill_loss.detach().cpu().numpy() * N, N)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epoch + 1):
        ...
        if args.train: 
            total_loss_train, recon_loss_train, distil_loss_train, mpjpe_train, dtw_train = train(config, train_dataloader, model, src_vocab, optimizer, clip_grad_fun)
            loss_epochs.append(total_loss_train * 1000)
            scheduler.step(epoch)
        with torch.no_grad():
            total_loss_test, recon_loss_test, distil_loss_test, mpjpe_test, dtw_test = test(config, test_dataloader, model, src_vocab)
